...
const Resources = () => {
    ...
    return (
        <div className="bg-white min-h-screen">
            {/* Header Section */}
            <section className="relative pt-24 pb-20 lg:pt-40 lg:pb-32 overflow-hidden bg-slate-900 text-white">
                ...
                <div className="container mx-auto px-4 relative z-10 text-center">
                    <span className="inline-block px-4 py-2 rounded-full bg-blue-600/20 border border-blue-400/30 text-blue-300 text-sm font-bold tracking-widest uppercase backdrop-blur-sm mb-6">Learning Hub</span>
                    <h1 className="text-4xl md:text-5xl lg:text-6xl font-black mb-8 leading-[1.1] tracking-tight">
                        Free <span className="text-blue-500">Resources.</span>
                    </h1>
                    <p className="text-lg md:text-xl text-slate-300 max-w-3xl mx-auto font-medium leading-relaxed">Enhance your learning with these curated online educational tools and platforms.</p>
                </div>
            </section>
                ...
                <div className="container mx-auto px-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {resources.map((category, index) => (
                            <div key={index} className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group">
                                ...
                                <div className="space-y-4">
                                    {category.items.map((item, idx) => (
                                        <a
                                            ...
                                            className="flex items-center justify-between p-5 rounded-2xl bg-slate-50 hover:bg-blue-50 transition-colors group/item"
                                        >
                                            <div>
                                                <h3 className="text-xl font-bold text-slate-800 group-hover/item:text-blue-700">{item.name}</h3>
                                                <p className="text-sm text-slate-500 line-clamp-1">{item.description}</p>
                                            </div>
                                            <ExternalLink size={18} className="text-slate-300 group-hover/item:text-blue-600 group-hover/item:translate-x-1 group-hover/item:-translate-y-1 transition-all" />
                                        </a>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="mt-24 p-12 bg-blue-50 rounded-[3rem] text-center border border-blue-100">
                        <h3 className="text-2xl font-black text-slate-900 mb-4 uppercase tracking-tighter">Important Note</h3>
